...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        def bfs(starts: List[Tuple[int,int]]) -> List[List[bool]]:
            ...
            while q:
                ...
                for di, dj in ((1,0),(-1,0),(0,1),(0,-1)):
                    ni, nj = i + di, j + dj
                    if 0 <= ni < m and 0 <= nj < n and not reachable[ni][nj]:
                        # can move from (ni,nj) -> (i,j) in forward direction if heights[ni][nj] >= heights[i][j]
                        # since we're doing reverse traversal, require heights[ni][nj] >= heights[i][j]
        ...
        for i in range(m):
            for j in range(n):
                if pac_reach[i][j] and atl_reach[i][j]:
                    ans.append([i, j])
        return ans
